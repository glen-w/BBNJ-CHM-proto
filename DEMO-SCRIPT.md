...
1. **Home `/`** as the anonymous public — rails with counts filtered by role: 0 pending packs, no notifications. Open **Institutional** (`/institutional`) for Secretariat notices and **Related systems** — say: "an early interoperability seam: named links, not federation". Same related-systems list also sits on **About this desk**; *Export CSV* there downloads exactly the rows the public can see. Journey cards are launchers (counts + Open), not essays. Recently published stacks title / status / id+date. Say: "counts, lists, exports and notifications all go through one SQL visibility clause". Honesty caption lives in About: *Plausible demo data; not real Party filings.*

1b. **Rich seed beat (optional, ~90 s)** — open `/search` and try the suggested queries (`TEMP`, `utilisation`, `Sargasso`, `Polar Front`). Confirm **Interim (DOALOS)** on the mirrored MGR hit (same badge on `/mgr`). Open it: DMP / artefact URL points at the live DOALOS TEMP page; B-SBI and `publicRecordId` are real Cl-HM ids (never equal to the TEMP label). Then `/mgr`: pending *Sargasso eDNA* (B-SBI, no public id); *Secretariat-assisted* Tonga-Kermadec notice (`assisted` channel chip); Indian Ridge cruise at **utilisation**; restricted Polar Front eDNA (visible as secretariat / stb, 404 as public). `/eia` → **Demo scenario** badges on rocket splashdown (draft published), mCDR/OAE (draft pending), mesopelagic fishery (screening only), Polar Front observatory cable (**decision + monitoring** published), Indian Ridge vent field (draft pending) — open each and note the **neighbourhood** list (≥2 published activities in that ABNJ box). Open mesopelagic → **§ Agreement basis** cites Zotero `6K6WPBFQ` as RFMO-gap / Part IV framing. `/abmt` → published *Sargasso Sea Core* stub; pending *Indian Ridge vent field*; open *CCZ … precautionary* for the ISA not-undermine caption. `/capacity` → SIDS sequencing pair with a facilitation note; unmatched legal-policy need and remote-sensing / ship-time offers so **Run shared-theme rule** stays 0 new. Say: "shared-theme join finds the pair; the note is what the Secretariat actually did; theme-join alone is not matchmaking." Say: "list and search badges separate Interim (DOALOS) mirrors from Demo scenarios so rocket/mCDR/mesopelagic are never read as filed notifications."

1c. **Literature (optional, ~45 s)** — open **Literature** in the journey bar (`/research`). Say: "Zotero stays the catalog. This desk stores the citation and a link, not the PDF, and it does not sync live." Filter **EIA**. Open the splashdown activity: **Related research** names De Lucia & Guo 2026; **Open** leaves the desk. Open the Sargasso ABMT stub: Gjerde et al. 2016 and the Ocean Nexus webinar. A record with no shared place says **No linked research** — that is the join, not an empty library. Settings → Desk turns the lane off; the tab and the panels disappear. Say: "a paper appears here only when it shares the journey and a place name in the seeded boxes. This is not a Party filing and not a second library."
...
6. **Offline / SIDS loop — Art 51.5 as a pattern** — as `secretariat`, `/mgr/import` → **Import sample (2 valid + 1 invalid)** → run page: two rows *accepted → pending* with their own B-SBIs, one *rejected* with field-level errors and the received values. **Download error report (.xlsx)** → the Data sheet holds only the failed row plus an *Error* column. Then the same loop for **EIA screening**: download `/api/template/eia-screening.xlsx`, import the EIA sample — accepted rows become activities with a pending screening pack that already carries its Art 31 outcome. Say: "this is the low-bandwidth, assisted-access pattern the Agreement asks for in Art 51.5 — demonstrated, not certified; `ACCESSIBILITY.md` is the gap analysis". (In the unattended demo the MGR row is corrected and re-imported: 1 accepted.) Runs appear under *Import runs* on `/audit`. Optional coda: still as `secretariat`, open the gear → **Settings → Speed** (or `/settings?tab=speed`) → **Run the loop** → the same closed loop costed on office broadband vs SIDS 3G vs satellite. Say: "the wire numbers are mocked from nominal bandwidth and RTT, only the parse is measured — it shows why the payload stays small, not what a field link does".
...
8. **EIA artifacts + STB** — `/eia` → *Sediment sampling, CCZ*: published screening, notice, draft EIA, pending decision; the published packs list **artifacts** (PDF / URL / note references — references, not a file store). Sign in as `stb` → **STB queue** (`/stb`, also in the rails when logged in as STB) → several published drafts (CCZ sediment, rocket splashdown, Polar Front cable) → one consolidated comment on the CCZ activity → that row leaves the queue with a link back; a second comment on the same version is refused (and logged). *Baseline survey, CCZ* shows a published screening coexisting with a draft draft-EIA. Note the **neighbourhood** schematic + list (same ABNJ box) — vocabulary diagram, not GIS tiles.
...
10. **CBTMT facilitation + close** — `/capacity`: needs, offers (plus the uploader's offer from step 3), match rows + `match_suggested` events; each seeded match carries a **facilitation note** from the Secretariat (human brokerage — "we introduced both focal points"); edit it as `secretariat`; **Run shared-theme rule** is idempotent (0 new). Say: "the rule is theme intersection only — facilitation is the human note, not the join." Then open `/exhibit` for the printable EOI one-pager, `/about` for features, or `/compare` for the full Session‑1 table with live deep links. Say: "the left column describes the interim DOALOS pages as we understand them from public materials — an appropriate interim posture, and a design contrast rather than a critique". The **Demo user path** (gear → Settings → Demo user) walks the same five beats for self-guided evaluators. Finally, as `secretariat` with `SANDBOX_RESET=1`, **Reset database** on `/audit` → home again with fresh seeds.

## Alignment with `npm run demo`

`scripts/demo.ts` runs on its own temp database and asserts, in order: seed → public rails → MGR draft/receipt → Party publish refused → Secretariat publish + owner bell → MGR import loop (2+1, error workbook, corrected re-import) → STB comment → seeded v2 material publish → shared-theme matching (idempotent) → digest run (second run inserts 0) → exports/tiers → **facilitation note (no new event; Party refused)** → **non-State uploader (offer accepted; MGR draft, publish, import refused = 3 rows)** → **ABMT stub (draft → pending → `BBNJ-ABMT-`)** → **EIA artifacts (seed count + new pack with two refs)** → reconcile/replay → reset. With `BASE_URL` it also checks `/`, `/about`, `/settings`, `/settings?tab=speed`, `/lab/speed` (redirect), `/compare`, `/exhibit`, `/audit`, `/mgr/import`, `/notifications`, `/capacity`, `/cbtmt` (redirect), `/abmt`, exports, record JSON/PDF, restricted 404, `/api/health`.
...
- "Header languages open the official BBNJ page in a new tab; the UI stays English — no machine translation. The masthead shows which treaty-text locale you picked (Arabic RTL on that strip only)."
- "On a pack page, § Agreement basis lists only the articles for that record's domain and stages. **Latest transaction** in the third chrome band is the latest outbox event your role can see — it moves when you switch role or publish."
- "Hands-on means clone or `docker compose` plus the five logins — there is no hosted public URL in this release."

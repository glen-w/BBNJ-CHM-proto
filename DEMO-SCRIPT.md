...
SANDBOX_RESET=1 npm run dev     # http://localhost:3000 — the env var shows the Secretariat's Reset sandbox button
```
...
10. **CBTMT + close** — `/capacity`: one need, one offer, one match row + `match_suggested` event; **Run shared-theme rule** is idempotent (0 new). Then `/compare`: the three Session‑1 functions, every row linking back into what you just did. Finally, as `secretariat` with `SANDBOX_RESET=1`, **Reset sandbox** on `/audit` → home again with fresh seeds.

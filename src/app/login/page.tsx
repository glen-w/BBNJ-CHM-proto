...
const BLURB: Record<string, string> = {
  "party.nfp": "Submits MGR notifications, EIA packs and capacity needs for demo Party XSD. Sees own drafts plus everything published.",
...
export default async function LoginPage({ searchParams }: Props) {
  ...
  return (
    <AppShell title="Sandbox logins" flash={flash}>
      <p className="max-w-3xl text-sm text-muted-foreground">
        Cookie-based demo session, no passwords. Any missing, malformed, unknown or inactive cookie is treated as the anonymous public.
        {p.kind === "user" ? (
          <>
            {" "}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {users.map((u) => (
          <form key={u.id} action={loginAction} className="flex flex-col justify-between rounded-lg border p-4">
            <input type="hidden" name="userId" value={u.id} />
            <input type="hidden" name="_return" value={ret} />
            <div>
              <div className="flex items-center justify-between">
                <span className="font-mono font-medium">{u.username}</span>
                <span className="rounded bg-muted px-1.5 text-xs uppercase">{u.roles.join(", ")}</span>
              </div>
